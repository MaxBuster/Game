...
public class ServerHandler {
	...
	public ServerHandler(Model model, PropertyChangeSupport pcs, Socket socket) {
		this.model = model;
		this.pcs = pcs;
		...
		try {
			...
			while (true) {
				...
				case Constants.VOTE:
					...
					model.vote(candidate, player.getPlayer_number());
//					pgi.vote(current_round, candidate); FIXME get this info elsewhere
					player.setDone(true);
					model.continue_game_if_all_players_done();
					break;
				default: break;
	...
	private void write_voter_dist() {
		...
		int[] message = new int[]{current_dist.get_mean_1(), current_dist.get_std_dev_1(),
				current_dist.get_mean_2(), current_dist.get_mean_2()};
